...
| `case/JH-2018-001.html` 等四頁 | true | 案件頁 |
...
| `legacy.html` | false | 周妤的舊協尋網站（已停止更新） |
...
已完成：全站三語內容、徵信錄謎題（雜湊比對）、常駐筆記本、管理員測試面板、`about/faq/contact/donate/privacy/accessibility` 六個機構制式頁面、首頁統計與狀態篩選、每頁獨立靜態檔架構、第六幕完整時序與音訊容錯。

尚未決定、需要人來拍板的舊有事項：

1. 早期草稿曾寫「五個案件」，但實際上只做了四個（JH-2018-001／JH-2022-002／JH-2025-003／JH-2026-004）——要補第五案，還是把措辭改成「四個」？
2. `JH-2026-004` 原本有一個 `summaryRevised`（09/04 取得新事證後修正）欄位，目前沒有任何觸發機制讀取它，本輪重構時已從 `case/JH-2026-004.html` 的資料中移除（避免留著死欄位）。要幫這個「第二次中段更新」設計觸發條件，還是就此作罷？
