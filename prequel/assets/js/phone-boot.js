...
(() => {
  ...
  // 跟持續存在的手機外框共用同一把旗標:玩家點瀏覽器圖示這個動作是真正的使用者
  // 手勢,直接嘗試進全螢幕;之後每一頁(包含這裡之後的整個前作、續作)都會在
  // 玩家下一次點擊時再嘗試重進一次全螢幕(換頁會自動退出,無法用程式自動恢復)。
  ...
  function mount() {
    ...
    root.innerHTML = `
      ...
      <div class="jh-boot__home" id="jh-boot-home">
        ...
        <div class="jh-home-indicator">
          <button type="button" class="jh-nav-btn" aria-label="返回">${BACK_SVG}</button>
          <button type="button" class="jh-nav-btn" aria-label="主畫面">${HOME_SVG}</button>
          <button type="button" class="jh-nav-btn" aria-label="多工">${RECENTS_SVG}</button>
        </div>
      </div>
    `;
